...
EXTERN void _Draw_ALine_88( FLT xo, FLT yo, FLT x1, FLT y1 )
{
   FLT Tmp, dx, dy, S, T;
   INT yi, yf;
   ...
   if ( dy<.001 )     // Horizontal
   {
      ...
      Ptr = ((PIXEL*)_RCst_.Base_Ptr) + (y+1)*_RCst_.Pix_BpS - 2*x;
      for( ; x<xf; ++x )
      { 
   ...
   if ( S<dy )    // >45'
   {
      ...
      S = dx/dy;     // <1.0
      T = 65536.0/(FLT)sqrt( 1.0 + (double)S*S );
      ...
      xo += S*( (FLT)yi-yo );
      xi = (INT)ceil( xo );      
      Ptr = ((PIXEL*)_RCst_.Base_Ptr) + (yi+1)*_RCst_.Pix_BpS - 2*xi;
      Err = (UINT)ceil( T*( (FLT)xi-xo ) );

      yf = (INT)ceil( y1 );

      if ( (yi>=_RCst_.Clips[3]) || (yf<=_RCst_.Clips[2] ) ) return;
      ...
      if ( dx>0.0 )
      {
         ...
         while( yf-- )
         {
            MIX( Ptr[-2], AA_Table[ (Err>>8) ] );
            MIX( Ptr[2], AA_Table[ 0xFF^(Err>>8) ] );
            MIX( Ptr[0], C0 );
            Ptr += _RCst_.Pix_BpS;
            Err += dErr;
            if ( Err>=tErr )
